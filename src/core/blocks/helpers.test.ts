...
describe('hashMatchesDifficulty', () => {
  test('returns true when the hash of a block matches the difficulty', () => {
    const difficulty = 1;
...
describe('hexToBinary', () => {
  test('returns a binary string of a hex string', () => {
...
describe('nonceGenerator', () => {
  test('starts generating nonces from 0 if argument is not provided', () => {
    const nonceIterator = nonceGenerator();

    expect(nonceIterator.next().value).toBe(0);
  });

  test('generates nonces in incrementing order', () => {
    const nonceIterator = nonceGenerator();
  ...
  test('generates the first nonce equal to the argument if provided', () => {
